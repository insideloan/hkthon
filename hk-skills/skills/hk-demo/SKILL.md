...
| 0:30 | 클릭 → "통화" 버튼 | 발표자A | agent UI | "이 분께 전화를 겁니다" → 통화 버튼 클릭 시 발신 |
| 0:45 | S1 happy path | (자동) | agent UI | AI 봇이 인사 → 상품 소개 → 가입 권유 → 고객 OK |
...
#### C. 브라우저/탭 셋업
- 발표 직전 chrome 창:
  - agent UI (`/`) — 관리자 화면 + 통화 화면. 오디오는 노트북 마이크/스피커로 처리.
- 마이크/스피커 권한 미리 허용해 둘 것
...
- ✅ **발표자 2명 권장** — 1명이 진행/내레이션, 1명이 agent UI 조작.
