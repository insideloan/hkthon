...
| Issue type | 확인 방법 |
|---|---|
| Backend (API) | `curl -X POST .../api/foo` 200 + JSON |
| Backend (DB) | `sqlite3 backend/app.db 'SELECT ...'` |
...
- `STACK.md` §5 schema와 일치 (type, snake_case, field 다 있음)
...
- ❌ **TEAM LOCK PR을 1명이 approve하고 merge하지 않기** — 모든 팀원 approve 필요.
- ❌ **Schema PR을 ORCH owner + 사용 모듈 owner 둘 다 approve 없이 merge하지 않기.**
