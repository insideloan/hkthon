...
- `reference/API.md` (AppSync GraphQL 계약 — 뮤테이션/구독/쿼리 응답 schema 대조용)
...
# 람다 단위 테스트
pytest lambda/orchestrator/tests
# 프론트엔드 로컬 dev 서버
cd frontend && pnpm dev &
```

브라우저 `http://localhost:3000` 정상 응답. 배포된 환경이 있으면 AppSync 엔드포인트 smoke 테스트도 병행.
...
pytest lambda/orchestrator/tests          # all pass
```
...
| Issue type | 확인 방법 |
|---|---|
| Orchestrator (GraphQL 뮤테이션/쿼리) | `curl -X POST https://<api-id>.appsync-api.<region>.amazonaws.com/graphql -H "x-api-key: <key>" -d '{"query":"..."}` + `reference/API.md` schema와 일치 |
| DynamoDB | `aws dynamodb query --table-name <table> --key-condition-expression "PK = :pk" --expression-attribute-values '{":pk":{"S":"CALL#<id>"}}'` |
| AppSync 구독 | 브라우저 DevTools → Network → WS → AppSync Subscription 프로토콜 메시지 payload 확인 |
...
### 3.6 VERIFY.md D섹션 — AppSync 구독 (해당 시, 3분)

- Chrome DevTools → Network → WS → AppSync Subscription 프로토콜 메시지 payload 확인
- `reference/API.md` §2 schema와 일치 (필드명, camelCase, 필수 필드 모두 있음)
- 이탈위험도(`onIndexUpdate.churnRisk`) 검증: `reference/CHURN-RISK-LEXICON.md` §5의 worked example 발화를 순서대로 입력 → churnRisk 궤적이 문서 값(±2)과 일치하는지. 사전 기반이라 같은 입력은 같은 점수여야 함(결정적).
...
- ❌ **GraphQL 스키마 PR을 BACKEND owner + 사용 모듈(DATA/FRONTEND) owner approve 없이 merge하지 않기.**
...
| B섹션 AppSync 응답 data 이상 | schema mismatch (FE/BE) 또는 resolver 버그 | `graphql/schema.graphql` + resolver 코드 대조, type 정의 통일 |
| C섹션 "한국어 어색" | 번역투 | 문구 자연스럽게 (CONVENTIONS §10) |
| D섹션 구독 이벤트 안 옴 | DynamoDB write 누락 또는 Streams 미설정 | Lambda orchestrator CloudWatch 로그 확인 |
