...
| 단계 | 무엇을 하나 | 결과 |
|---|---|---|
| 1 | GitHub 계정 만들기 | github.com/your-name 계정 |
| 2 | GitHub CLI 설치 + 인증 | `gh auth status` OK |
| 3 | hk-skills 받기 | 본인 노트북에 폴더 |
| 4 | 본인 모듈 등록 | git config에 모듈 기록 |
| 5 | 작업 브랜치 만들기 | main에서 분기 |
| 6 | 파일 수정 (스킬/문서) | 본인 모듈 안에서만 |
| 7 | 변경 내용 stage + commit | git history에 기록 |
| 8 | push + PR 만들기 | 검토 요청 |
| 9 | 리뷰 받기 + 머지 | main에 반영 |
| 10 | 본인 브랜치 최신화 | 다음 작업을 위해 |
...
**Claude가 할 일**:
1. 사용자에게 이메일·이름·사용자명 확인
2. https://github.com/signup 안내
3. **중요**: GitHub 사용자명을 메모해두기 (예: `park-minji`)
4. **중요**: 이메일 인증 끝낼 때까지 대기
5. 인증 끝났으면 다음 단계로

**사용자가 직접 할 일**:
- https://github.com/signup 접속
- 이메일·비밀번호·사용자명 입력
- 이메일 인증 메일에서 "Verify email address" 클릭

**확인**:
- https://github.com/<본인-사용자명> 페이지가 보이면 OK

---

### STEP 2. GitHub CLI 설치 + 인증
...
### STEP 3. hk-skills 받기 (clone)

**Claude에게 할 말**: `팀 매뉴얼 step 3 따라해줘`

**Claude가 할 일**:
1. 어디에 받을지 사용자에게 확인 (기본: `~/workspace/hk-skills`)
2. 원본 repo URL 확인: `https://github.com/insideloan/hkthon.git`
3. **Fork** 안내 (본인 계정으로 작업하려면):

   **방법 A (Fork) — 권장**:
   ```bash
   # 1) GitHub에서 직접 fork
   #    https://github.com/insideloan/hkthon 페이지에서 "Fork" 버튼 클릭
   #    → 본인 계정(hkthon)으로 fork됨

   # 2) 본인 fork clone
   gh repo clone <본인-사용자명>/hkthon ~/workspace/hkthon
   cd ~/workspace/hkthon
   ```

   **방법 B (직접 clone) — collaborator인 경우만**:
   ```bash
   gh repo clone insideloan/hkthon ~/workspace/hkthon
   cd ~/workspace/hkthon
   ```

4. **올바른 디렉토리**에 들어왔는지 확인:
   ```bash
   pwd
   ls
...
5. 원격 정보 확인:
   ```bash
   git remote -v
   # origin  https://github.com/<본인-사용자명>/hkthon.git (fetch)
   # origin  https://github.com/<본인-사용자명>/hkthon.git (push)
   ```

**문제 해결**:
- `Permission denied`: collaborator가 아님. 팀 리더에게 추가 요청
- `Repository not found`: 사용자명 오타. `gh repo list`로 본인 repo 확인

---

### STEP 4. 본인 모듈 등록 (git config)

**Claude에게 할 말**: `팀 매뉴얼 step 4 따라해줘`
...
**Claude에게 할 말**: `팀 매뉴얼 step 5 따라해줘. <수정-목적-한-줄>`
...
**Claude에게 할 말**: `팀 매뉴얼 step 6 따라해줘. <파일-경로> 수정할 거야`
...
3. **모듈 boundary 확인** (Claude가 자동):
   ...
   - 안 속하면 → 다른 사람 것. **PR로 요청** (Step 7-9) 또는 팀 리더에게 음성
...
6. PR URL을 사용자에게 보여줌 (예: `https://github.com/<본인>/hkthon/pull/42`)
...
**Claude에게 할 말**: `팀 매뉴얼 step 10 따라해줘`
...
**Q. 왜 fork를 해야 하나요?**
A. 본인 계정으로 안전하게 작업 + PR을 원본에 요청하는 표준 방식.
...
**"처음부터 끝까지 다 해줘"**:
- "팀 매뉴얼 step 1부터 step 10까지 다 따라해줘"
- "git 설치부터 PR 머지까지 가이드해줘"
...
**"문제 해결"**:
- "hook이 push를 막았어" → Step 6의 모듈 boundary 섹션 참고
- "merge conflict 났어" → Troubleshooting 참고
- "PR이 안 보여" → `gh pr list` 실행

**"다른 사람 도움"**:
- "OO의 PR 리뷰 좀 봐줘" → Step 9의 reviewer 섹션
...
이 매뉴얼을 따라 한 번 작업해보세요. 가장 좋은 학습은 **직해보는 것**입니다.
