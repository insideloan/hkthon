...
>
> **변경 규칙**: REST/WS schema는 **BACKEND가 정의**합니다(데이터 모양은 DATA, 클라 타입은 FRONTEND와 합의). 변경은 BACKEND PR + 사용 모듈 owner 통보
> (`docs/WORKFLOW.md` §3.1 schema 변경 프로토콜).
>
> **포맷 안내 / Formats**:
> - 본 문서(`API.md`)가 **design SSOT** — REST + WebSocket + 화면 매핑을 사람이 읽기 위한 형식.
> - `reference/openapi.yaml` = REST subset의 **machine-readable** 버전 (codegen/contract-test/mock용). §1과 항상 동기화 (변경 시 둘 다 수정 → BACKEND PR).
...
### 1.1 대시보드 / 큐 (BACKEND 정의, FRONTEND 사용)
...
### 1.2 통화 / Calls (BACKEND 정의)

#### `POST /api/calls/start`
발신 트리거. 사전 고객분석 후 **"통화" 버튼 클릭 시** 콜을 생성하고 `DIALING`으로 전환. (FRONTEND의 통화 버튼이 호출. **행 클릭만으로 자동 발신하지 않음.**)
...
통화 종료. `state → ENDED` 후 AI 인계 요약 생성 트리거 (→ §1.5). 모니터링 화면의 "종료" 또는 음성 채널(`/ws/audio` hangup)에서 호출.

**200**: `Call` (state=`ENDED`)

### 1.3 고객 / Customers (BACKEND 정의, FRONTEND 사용)
...
### 1.5 인계 요약 / Summaries (BACKEND 정의, AGENT 생성)

#### `POST /api/summaries`
통화 종료 시 AI가 생성한 인계 요약 저장. AGENT가 `call_ended` 후 요약을 생성해 호출(콜백).
...
> 모든 프레임: `{ "type": "...", ...payload }`. 공유 타입은 `frontend/src/types/ws.ts` (FRONTEND 소유).
...
| type | payload | 페이로드 생산 | 설명 |
|---|---|---|---|
| `queue_update` | `{ rows: QueueRow[], summary: {...} }` | BACKEND | 큐 리스트 + 요약 카드 갱신 |
| `call_started` | `{ call_id, customer: Customer }` | BACKEND | 콜 시작 |
| `transcript` | `{ call_id, speaker, text, node_id, ts }` | AGENT(STT) | 발화 1건 (`speaker: bot \| customer \| agent`) |
| `node_entered` | `{ call_id, node_id }` | AGENT | LangGraph 노드 진입 → 그래프 하이라이트 |
| `index_update` | `{ call_id, churn_risk, emotion }` | AGENT | 이탈위험도/감정 게이지 |
| `guidance` | `{ call_id, text, reason }` | AGENT | AI Action 추천 Comment + 판단 근거 |
| `ai_action` | `{ call_id, comment, action }` | AGENT | 중앙 메인 AI Action 강조 |
| `fraud_flag` | `{ call_id, fraud_suspected }` | AGENT | 금융사기 의심 감지 → 대시보드 강조/카드 (통화 계속) |
| `call_ended` | `{ call_id, result_type }` | BACKEND | 종료 → 요약 화면 트리거 |
| `error` | `{ code, message, fallback_message }` | (전체) | 외부 API 실패 fallback |

> 모든 메시지의 구독·표시(consumer)는 FRONTEND. 분석 이벤트(`transcript`/`node_entered`/`index_update`/`guidance`/`ai_action`/`fraud_flag`)의 값은 AGENT가 생산하고 BACKEND가 중계.

**Client → Server (`AgentCmd`)** — 모두 FRONTEND가 송신, BACKEND가 처리

| type | payload | 설명 |
|---|---|---|
| `approve_product` | `{ product_id }` | 가입 승인 (= `POST /approve`의 WS 변형) |
| `transfer_to_agent` | `{}` | 상담원 연결 (S1) |
| `send_link` | `{ url }` | 문자URL발송 |
| `mic_speaker` | `{ speaker: "agent" \| "customer" }` | 마이크 화자 토글 (데모: 단일 마이크) |
| `hangup` | `{}` | 통화 종료 |
| `next_in_queue` | `{}` | 다음 대기콜 |

### 2.2 `/ws/audio` — 음성 채널 (마이크/스피커) ↔ backend

> 고객 iPhone UI 폐지로 별도 고객 탭은 없음. 오디오는 상담원 화면이 직접 주고받으며, 채널 소유는 ORCH.

연결: `ws://localhost:8000/ws/audio?call_id=<id>`.
...
| `call_ended` | `{ call_id }` | 통화 종료 신호 |

**Client → Server**

| type | payload | 설명 |
|---|---|---|
| `start` | `{}` | 통화 시작 → `state=ACCEPTED` → STT 스트림 시작 |
| `hangup` | `{}` | "종료" → `state=ENDED` |
| `audio_chunk` | `{ seq, format: "webm", b64 }` | 마이크 2-3초 청크 → Transcribe STT |
...
| 화면 (PRODUCT-BRIEF §4) | REST | WS | 화면(소유) / 데이터(제공) |
|---|---|---|---|
| 관리자 대시보드 (요약 카드 + 큐) | `GET /api/queue` | `/ws/agent` (`queue_update`) | FRONTEND / BACKEND |
| 실시간 통화 모니터링 | `GET /api/calls/{id}`, `GET /api/customers/{id}`, `POST /api/calls/{id}/{approve,transfer,send-link,hangup}` | `/ws/agent` (`transcript`/`node_entered`/`index_update`/`guidance`/`ai_action`) | FRONTEND / BACKEND + AGENT |
| 통화 종료·인계 요약 | `GET /api/calls/{id}/summary`, `POST /api/summaries` | `/ws/agent` (`call_ended`) | FRONTEND / BACKEND + AGENT |
| 음성(STT/TTS) | — | `/ws/audio` (`audio_chunk`/`audio_out`) | FRONTEND / BACKEND 중계 + AGENT(STT/TTS) |

> 새 endpoint/메시지 추가 시: 이 문서 + `docs/MODULES.md` §5 + `frontend/src/types/ws.ts` 동시 갱신 → BACKEND PR.
...
- ❌ 실제 전화망/Twilio — 오디오는 `/ws/audio`로 노트북 마이크/스피커를 통해 시뮬레이션.
- ❌ 멀티테넌시/페이지네이션/rate-limit — 단일 상담원 데모 범위.
