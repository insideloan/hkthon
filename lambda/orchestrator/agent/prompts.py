...
STAGE_GUIDE: dict[Stage, str] = {
    Stage.IDENTIFY: """\
[STEP 1 — 신원고지/녹취고지 + 연락 근거]
목표: 본인 확인과 녹취 고지를 마치고, 연락 근거(마케팅수신동의)를 사실대로 밝힌 뒤 상품제안으로 자연스럽게 연결.
★ 이 통화는 당사가 먼저 건 아웃바운드 전화이므로, 고객 정보를 이미 보유하고 있다.
  [고객 정보]의 이름을 그대로 불러 본인 확인을 한다 — 고객에게 이름을 되묻지 않는다.
  본인 확인은 이름 확인만으로 한다(생년월일·주민번호 등 추가 신원 정보를 되묻지 않는다).
...
★ 본인 확인이 끝나면 연락 근거를 사실대로 밝힌다: "마케팅 및 개인정보 활용(마케팅수신동의)에
  동의해주셔서 대출상품 안내차 연락드렸다"고 안내하고, "지금 통화 잠깐 괜찮으신지"를 먼저 여쭌다.
대응전략(케이스별):
- 본인확인됨: 확인 감사 인사 후 연락 근거(마케팅수신동의)를 밝히고 통화 가능 여부를 여쭌 뒤 상품제안으로 연결.
...
- 번호출처/연락경위 의심("제 번호를 어떻게 아셨나요"): 고객이 사전에 마케팅수신동의(마케팅 및 개인정보 활용 동의)를
  하셔서 그 정보를 근거로 연락드렸음을 사실대로 안내(과도한 출처 언급 지양).
- 동의사실 인지 못함: 마케팅수신동의 시점/경로 등 기본 사항으로 안내. 상세 안내 요청 시 상담원 연결.
...
금지/주의: 첫 인사에서 신원·AI음성/녹취·본인확인 3종 중 어느 것도 누락 금지, 본인확인은 이름 확인만(생년월일·주민번호 추가 확인 금지), 본인 아님에 정보 캐묻기 금지(제3자 개인정보 노출), 녹취를 끄겠다는 거짓 안내 금지, 동의 사실 추측·단정 금지, 철회 만류 금지, 고객에게 대출 자금 용도·사유 되묻기 금지(아웃바운드 안내 전화), 추측성 답변 금지, 신원고지 단계라도 거절 의사 최우선 처리.""",

    Stage.PROPOSE: """\
[STEP 2 — 상품제안(적합성/중요사항)]
