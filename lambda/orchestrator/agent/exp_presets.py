...
EXP_PRESETS: dict[Intent, ExpPreset] = {
    ...
    Intent.TRANSFER_INTENT: ExpPreset(
        emotion="짜증", need="본인 확인", usability="AI 접수 필요",
        tactic="AI 접수 전환 전략", headline="상담원 요청 — AI 본심사로 전환",
        rationale="상담원을 요청했지만 사람 연결은 없다. AI가 직접 본심사를 진행한다.",
        tokens=[_tok("사람 바꿔주세요", "NEUTRAL", "AI 본심사 전환")],
        db_chips=["고객 프로필", "신용평가"],
        db_nodes=[_node("본심사 접수", "AI 진행 중", "pos"), _node("처리 경로", "AI 본심사", None)],
        compliance_draft="네, AI 상담사가 직접 본심사를 진행해 드리겠습니다.",
        compliance_violations=[],
    ),
    Intent.LIMIT_INQUIRY: ExpPreset(
        emotion="기대", need="한도 확인", usability="AI 접수 필요",
        tactic="AI 접수 전환 전략", headline="한도 문의 — AI 본심사로 즉시 접수",
        rationale="대출 가능 한도를 알고 싶어한다. AI가 직접 본심사 접수로 안내한다.",
        tokens=[_tok("얼마까지 나와요?", "PRO", "한도 확인 니즈")],
        db_chips=["신용평가", "대환 한도"],
        db_nodes=[_node("예상 한도", "심사 후 확정", None), _node("접수 경로", "AI 본심사", "pos")],
