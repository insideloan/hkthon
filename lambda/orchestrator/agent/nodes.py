...
def transfer_node(state: CallState) -> CallState:
    """상담원 이관. transferToAgent 경로 → state=TRANSFER_PENDING. 이관 멘트 + 대화요약 준비."""
...
def close_node(state: CallState) -> CallState:
    ...
    # TODO: state=ENDED 트리거 (endCall은 별도 뮤테이션)
    return {"bot_text": text, "stage": Stage.CLOSING}


def silence(state: CallState) -> CallState:
    """무응답/침묵: 10초 재확인 1회. 연속 무응답 2회↑면 종료, 3회 재시도 금지."""
    silence_streak = _count_trailing_silence(state)
    if silence_streak >= 2:
        # TODO: state=ENDED
        return {"bot_text": "연결 상태가 좋지 않은 것 같습니다. 다음에 다시 연락드리겠습니다.", "stage": Stage.CLOSING}
