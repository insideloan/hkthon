...
## 1. 📋 Happy Path 시나리오 (Full 24h, 4명 무사 귀환)
...
1. "한 문장으로, 이게 뭔가요? 심사위원이 10초 안에 기억할 한 문장."
   → 수민: "콜센터 상담원이 누르면 AI가 알아서 금융상품 판매 전화를 거는 시스템입니다."

2. "사용자는 누구인가요?"
   → 지훈: "1차 사용자는 상담원이고, 2차 사용자는 (가짜) 고객이에요. 심사위원은 시청자."

3. "핵심 시나리오 3개는?"
   → 예린: "가입 희망, 분노 에스컬레이션, 사기 의심. 셋 다 상담원 인계로 끝나요."

4. "데모 환경은?"
   → 도윤: "노트북 로컬, 1세트. 클라우드 배포 안 합니다."

5. "이번 24시간에 명시적으로 안 할 것은?"
   → 팀 합의: "실제 전화망, 인증, 영문, 다중 상담원, 모바일 반응형."
```

**산출물**: `reference/PRODUCT-BRIEF.md` 자동 갱신. 4명 모두 보고 "OK".
...
**시간**: 30분 × 4명 (병렬). 모두 끝나면 `git init` + initial commit.

### 1.2 Phase 2 — Backlog & Slice (1:00 - 3:00, 2시간)

#### [1:00] `/hk-backlog` (4명 함께)
...
| F03 | S1 happy path state machine | S1 | BE | D | 2.0 | P0 |
| F04 | Call graph + transcript panel | S1/S2/S3 | FE | C | 2.0 | P0 |
| F05 | Persona/credit/product approval | S1 | FE+BE | C | 1.5 | P0 |
| F06 | S2 anger detection node | S2 | BE | D | 1.5 | P0 |
| F07 | S3 fraud detection node | S3 | BE | D | 1.5 | P0 |
...
| F12 | Demo seed data (3 시나리오용 1명씩) | cross | BE | D | 0.5 | P0 |
| F13 | TTS 음성 폴리시 (mijin) | polish | BE | D | 0.5 | P1 |
| F14 | State machine 노드 그래프 시각화 polish | polish | FE | C | 1.0 | P1 |
| F15 | Demo fallback (LLM timeout 시 hardcoded) | polish | BE | D | 1.0 | P1 |

**P0만 = 12개, 약 16h**. P0+P1 = 18.5h. 24h 안에 P0 + 대부분 P1 가능.

`BACKLOG.md` 작성, 4명 모두 "OK" → 합의.

#### [1:30] `/hk-slice` (F01 → F05 순서로, 4명 함께)
...
- id: QUEUE-001-outbound-table-component
  ...
  acceptance:
    - 페이지에 row 10개 표시 (mock data)
    - 색상 분기 (yellow/green/red) 동작
    - row 클릭 시 /call/[id]로 이동
...
[4:45] Acceptance 자가 검증:
  ...
  - 색상 분기 동작 OK (개발자 도구로 className 확인)
...
[5:00] gh pr create --title "[QUEUE] outbound table component" --reviewer @yelin
[5:02] Issue: in-progress → in-review
[5:03] Hand-off 메시지: "✅ QUEUE-001 구현 완료, PR #1, Reviewer @yelin"
```

#### [3:00-21:00] **4명이 위 루프를 18시간 동안 반복**

대략적인 분포:
- **수민 (A, QUEUE)**: 6 issue, 약 6-7시간 코드
- **지훈 (B, PHONE)**: 5 issue, 약 5-6시간
- **예린 (C, CALL+MEMO)**: 8 issue, 약 8-10시간
- **도윤 (D, ORCH)**: 7 issue, 약 7-9시간

#### [3:00] **ORCH schema PR이 가장 먼저 머지되어야 함 (ORCH-001-schema-and-ws)**

도윤이 `ORCH-001`를 가장 먼저 시작. `backend/app/ws/agent_ws.py`, `backend/app/ws/customer_ws.py`, `frontend/src/types/ws.ts`에 schema 정의.

**PR #2 [ORCH] WS schema + agent/customer WS**:
- 도윤이 raise, reviewer: 수민 (QUEUE 사용), 지훈 (PHONE 사용)
...
`/hk-verify`의 §8 통합 단계:
- S1, S2, S3 풀 플로우를 1명이 처음부터 끝까지 1번씩 클릭
...
| 1:30 | 인계 | 예린 | agent UI call/[id] |
| 1:50 | 노드 그래프 | 예린 | call view 좌측 |
| 2:10 | 페르소나 + 승인 | 예린 | call view 우측 |
| 2:30 | 메모 | 예린 | popup |
| 2:50 | S2 (분노) 압축 | (자동) | 양쪽 |
| 3:20 | S3 (사기) 압축 | (자동) | 양쪽 |
| 3:50 | 마무리 | 수민 | 슬라이드 |
...
폴리시 5개 영역 점검:
- **A 시드**: `python -m app.seed` → 고객 10명 (S1×3, S2×3, S3×3, normal×1)
...
- 슬라이드 전환이 5초 늦음
- S2/S3 압축 데모에서 1번씩 멈칫
- **discussion 5분**:
  - 잘 된 것: queue 색상, 노드 그래프
  - 막힌 것: S2에서 "분노" 트리거가 1번 실패 (LLM JSON parse)
  - polish: 슬라이드 단축, S2/S3 5초씩 더 짧게
...
3. `/hk-slice`로 ORCH owner(도윤)에게 handoff
4. 도윤이 schema PR → reviewer: 수민 + 지훈 → 머지
...
**시간**: [10:00], 도윤의 ORCH-003 (state machine S1) PR이 떠 있음.

**상황**:
- 도윤: PR #5 [ORCH] S1 state machine. Reviewer: 수민, 예린.
- 수민: 자기 QUEUE-003 작업 중 (3시간 짜리 큰 issue, 거의 끝나감)
- 예린: 30분간 자리비움 (점심)
- 1시간 후: review 0건
- 도윤: 슬랙 ping → 응답 없음
- 1.5시간 후: 도윤이 음성으로 "PR #5 review 부탁" 알림
- 수민: "아 미안 지금 끝나면 볼게" → 30분 더
- 2시간 후: 수민 approve + 머지
- 예린: 1시간 후 approve (이미 머지됨)

**영향**:
- 예린의 CALL-002 (call graph)가 ORCH schema를 기다리며 **2시간 blocking**
- 예린: status:in-progress issue는 1개만 가지니까 → 다른 이슈 진행. 1.5시간은 다른 곳에서 못 박음.

**학습**:
- 1h SLA는 의도. 24h에 1시간은 길다.
- 도윤은 30분 후 voice ping으로 **즉시 알림**했어야 함.
- 예린은 PR이 안 머지되면 issue status를 `status:blocked`로 표시.
...
**시간**: [15:00], 예린이 CALL-003 끝내고 CALL-004 시작. 1시간 후 지쳐서 다른 P1 (CALL-009 polish)도 시작.

**상황**:
- 예린: status:in-progress = CALL-004, CALL-009 (2개)
- GitHub Project 보드에서 빨간 라벨 (위반)
- 22h 시점에 CALL-004가 80% 완성, CALL-009는 50%

**수민의 발견**:
- 22h sync에서 "야 예린, in-progress 2개 아닌가?"
- 예린: "아 그러네" → CALL-009 close (P1이라 시간 없음)
...
**시간**: [14:00], 도윤이 ORCH-007 (call transcript schema 변경) push + 머지. **CALL 모듈(예린)은 모름.**

**상황**:
- 도윤: `transcript` WS 메시지에 `confidence` 필드 추가
- 머지 30분 후
- 예린: CALL 화면에서 transcript panel 안 보임 (TypeScript error: `confidence` required)
...
**예린의 대응**:
1. `git log origin/main` → 도윤의 schema PR 발견
2. 음성으로 도윤에게 ping: "야 schema 변경 합의 있었어?"
3. 도윤: "아 미안, 5분 voice로 설명할게" → 합의 (confidence optional로 변경)
4. 도윤이 다시 PR로 optional로 fix
5. 예린은 type fix 후 작업 재개

**학습**: WORKFLOW.md §3.1에 "Schema 변경 PR은 ORCH owner + 사용 모듈 owner 2명 approve" 명시. 도윤이 이를 무시. 24h에 흔한 실수.
...
**시간**: [16:00], 도윤·수민·예린 3명이 각각 ORCH/QUEUE/CALL PR에서 `backend/app/main.py` 수정 (router include).

**상황**:
- 수민: `app.include_router(queue_router)` 추가
- 예린: `app.include_router(calls_router)` 추가
- 도윤: `app.include_router(memos_router)` 추가
...
**시간**: [19:00], 예린이 CALL-005 (product approval) PR. Reviewer: 수민 (QUEUE 사용).
...
- **팀 합의**: Option B.
  - 4명 모두 P0 in-progress 1개씩 +1 → 총 4-5개 더 끝냄 (각 1-1.5h)
  - 남는 1-2개 P0는 close (이번엔 skip, 다음 해커톤에서)
  - 통합 2시간, 데모 2시간
...
**상황**: 도윤이 ORCH-009 (긴급 fix) push 시 hook이 막음. **2시간 소모 후 30분 절약하려고** `--no-verify`.

**결과**:
- main에 PHONE 모듈 파일 + CALL 모듈 파일이 섞여 들어감
- 이후 4명이 rebase 시 1시간씩 lost
...
**상황**: 예린의 CALL-006 PR이 2일째 review 안 됨 (수민·도윤 busy).

**결과**:
- 예린: 다음 issue 못 시작, 1.5일 lost
...
**상황**: 지훈이 PHONE-007에서 `react-spring`, `framer-motion`, `@use-gesture/react` 등 5개 dep 추가 시도.

**결과**:
- TEAM LOCK PR 1개로 합의 시도 → 4명 중 2명 반대
- 1시간 lost
...
- 도윤이 ORCH-001 in-progress, ORCH-002 in-progress (2개)
...
### 3.6 Anti: 4명이 동시에 같은 모듈 (CALL)

**상황**: 예린(C) + 도윤(D, ORCH이지만 CALL schema 작업)가 동시에 CALL의 frontend 파일 수정.
...
4. queue 색상 (yellow/green/red)은 `CONVENTIONS.md` §6.2대로 보존
...
5. 새 owner 배정: 4명 중 1명이 ANALYTICS + 기존 모듈 이중 owner (불가피)

**시간**: 1시간. 24h에 새 모듈 추가는 거의 없지만, 가능.

---

### 4.4 5명 팀 (5 modules, 1 hub)

**상황**: 한 명이 추가로 합류. 5번째 모듈 `ADMIN` (관리자 화면) 추가.

**진행**:
1. `docs/MODULES.md` §1 + §2 yaml 갱신
2. `OWNER.md` 갱신
3. 5명 매핑: Person A→QUEUE, B→PHONE, C→CALL+MEMO, D→ORCH, E→ADMIN
4. setup-project.sh --module ADMIN 실행 (Person E)
5. Pre-push hook: Person E는 ADMIN만 push 가능

**시간**: 30분. hk-skills는 4-5명 모두 지원 (README §1).
...
- [ ] 4명 동시 같은 모듈: OWNER.md sequencing
...
| 9 | 5명 팀 시 pre-push hook 재설치 필요 | 4.4 | `setup-project.sh --module ADMIN`이 한 번에 처리 |
...
이 시나리오 문서를 **실제 해커톤 전에** 팀 4명과 함께 walkthrough:
1. 4명이 각자 자기 모듈 시나리오를 읽고 "이거 맞나?" 확인
