...
| # | 항목 | 결정 | 근거 |
|---|------|------|------|
| 1 | 단계 모델 SSOT | **xlsx 단계** 채택 (`IDENTIFY → PROPOSE → CHANNEL` + 공통요건). 동의/목적안내(CONSENT)는 IDENTIFY로 흡수 | 실제 콜 플로우가 가장 구체적 |
| 2 | 산출물 | 설계 + 스켈레톤 코드 | — |
...
### 0.1 xlsx 단계 ↔ ARCHITECTURE.md S1 노드 매핑
...
|-----------|---------------------------|------------------------|-----------|
| STEP 1 신원고지/녹취고지 + 연락 근거 | `IDENTIFY` | `GREETING` + `INTRO_PRODUCT` | 본인확인 OK·통화동의 → PROPOSE / 본인아님·거부 → CLOSING |
| STEP 2 상품제안(적합성/중요사항) | `PROPOSE` | `HANDLE_OBJECTION` + `OFFER_SIGNUP` | 진행의사 → CHANNEL / 최종거절 → CLOSING |
| STEP 3 채널선택 | `CHANNEL` | `OFFER_SIGNUP` 종단 | 셀프 → 링크발송·종료 / 상담원 → TRANSFER |
...
> ⚠️ **구현 현황 (2026-06-26): 동의/목적안내(`CONSENT`) 단계는 폐기되어 `IDENTIFY`에 흡수됨.**
> 본인 확인은 **이름 확인만**으로 한다(생년월일·주민번호 등 추가 신원 확인 없음). 연락 근거 고지
> — 고객이 사전에 **마케팅수신동의(마케팅 및 개인정보 활용 동의)**를 하셔서 그 정보를 근거로
> 연락드렸다는 안내 — 는 별도 `CONSENT` 단계나 고정 멘트가 아니라 **`IDENTIFY` stage 지침 + 공통요건**으로
> `respond`가 LLM 생성한다. "제 번호를 어떻게 아셨나요" 같은 번호출처/연락경위 의심도 같은 마케팅수신동의
> 근거로 응대한다.
>
> `state.stage`는 진입 단계 추론값이고, 실제 상담 진행 SSOT는 **`ConvFlow`(conversation_flow.py)** 이다 —
> `identity_confirmed → availability_confirmed → offer_made → loan_interest_answered`(1~4단계 순차)
> + `rejection_count` 방어/종료 축. `route_intent`·`fast_route`·`close_node` 모두 `flow`로 분기한다.
> `nodes.py`가 `stage`를 쓰기하는 곳은 종료 시 `Stage.CLOSING`으로 바꾸는 종단 노드뿐이다.
...
| `stage` | `Stage` | load_context/classify | 현재 단계 (IDENTIFY/PROPOSE/CHANNEL/CLOSING) |
...
- `IDENTIFY`: 신원·녹취 고지 의무, 본인확인은 이름 확인만(생년월일·주민번호 추가 확인 금지), 본인아님 시 정보발설 금지, 녹취 임의중단 불가. 연락 근거(마케팅수신동의) 안내·통화동의 확인, 보이스피싱 의심 시 안심보다 객관적 확인경로, 동의철회 즉시 접수.
