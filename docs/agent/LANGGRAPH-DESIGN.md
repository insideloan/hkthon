...
> ⚠️ **구현 현황 (2026-06-26): `Stage` 4단계 전진은 의도적으로 폐기됨.**
> 위 표의 `IDENTIFY → CONSENT → PROPOSE → CHANNEL` 전진 로직은 **코드에 구현하지 않는다.**
> `nodes.py`에서 `stage`를 쓰기하는 곳은 종료 시 `Stage.CLOSING`으로 바꾸는 종단 노드뿐이고,
> `context._infer_stage()`는 직전 봇 Turn의 `node`를 되읽으므로 통화 내내 `IDENTIFY`에 고정된다.
> 실제 상담 진행은 `Stage` 대신 **`ConvFlow`(conversation_flow.py)** 가 단독으로 끌고 간다 —
...
> 따라서 `state.stage`는 사실상 vestigial 필드이며, **`Stage`를 진행 모델로 신뢰하지 말 것.**
>
> 결과로 죽어 있는 코드 경로 두 가지(의도된 동작):
> - `prompts.py`의 stage별 가이드(`CONSENT`/`PROPOSE`/`CHANNEL`)는 주입되지 않는다 — 항상 `IDENTIFY` 프롬프트.
> - `nodes._consent_disclosure()`는 `stage == CONSENT`에서만 발동하므로 **고정 동의 고지 멘트는 그래프에서 나가지 않는다.**
>   동의 고지가 필요한 경우 `respond` 프롬프트(공통요건)와 `ConvFlow` 기반 처리로 대체한다.
